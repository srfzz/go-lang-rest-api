...
func deleteEvent(context *gin.Context) {
	slog.Info("EneteredDeleting the Event Route")
	eventId, err := strconv.ParseInt(context.Param("eventId"), 10, 64)
	if err != nil {
		context.JSON(http.StatusBadRequest, gin.H{"message": "unable to parse error :" + err.Error()})
		return
	}
	err = models.DeleteEvent(int(eventId))
	if err != nil {
		context.JSON(400, gin.H{"message": "Error:" + err.Error()})
		return
	}
